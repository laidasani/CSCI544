...
def Tokenize(comment):
    ...
    # Splits on non-letter
    words = re.findall('[A-Za-z]+', comment)

    # Convert to lower case
    # Drop words less than 2 char
...
def FirstLayer(net, l2_reg_val, is_training):
    ...
    # Preprocess the layer input
    net = tf.nn.l2_normalize(net, axis=1) # Normalize each row of tensor
    
    net = tf.contrib.layers.fully_connected(
            net, 40, activation_fn=None, 
            biases_initializer=None)

    # L2-regularization term 
    r_term = l2_reg_val*tf.reduce_sum(tf.multiply(net, net))
...
def EmbeddingL2RegularizationUpdate(embedding_variable, net_input, learn_rate, l2_reg_val):
    ...
    # Normalize each row of tensor
    net_input = tf.nn.l2_normalize(net_input, axis=1) 

    expr = 2*l2_reg_val*tf.matmul(tf.transpose(net_input),
                                tf.matmul(net_input, embedding_variable))

    return embedding_variable.assign(embedding_variable - learn_rate*expr)
...
def EmbeddingL1RegularizationUpdate(embedding_variable, net_input, learn_rate, l1_reg_val):
    ...
    inter = tf.matmul(net_input, embedding_variable)
    expr = l1_reg_val*tf.matmul(tf.transpose(net_input), inter/tf.math.abs(inter))
...
def SparseDropout(slice_x, keep_prob=0.5):
    ...
    """
    #non_zero_features, = numpy.where(numpy.any(slice_x != 0, axis=0))
    
    #select = numpy.random.choice(non_zero_features, 
    #                size=int((1.0-keep_prob)*non_zero_features.shape[0]),
    #                replace=False)

    #slice_x[:,select] = 0.0

    non_zero_features = numpy.transpose(numpy.nonzero(slice_x))
    
    # Select random rows
    select = numpy.random.choice(non_zero_features.shape[0],
                            size=int((1.0-keep_prob)*non_zero_features.shape[0]), 
                            replace=False)

    # Pick coordinates to replace
    replace = non_zero_features[select,:] 

    slice_x[replace[:,0],replace[:,1]] = 0.0
...
def ComputeTSNE(embedding_matrix):
    ...
    """
    embed = TSNE(learning_rate=50, n_iter=500).fit_transform(embedding_matrix)
...
def VisualizeTSNE(sess):
    ...
    class_id = [] # For coloring points
    x1_coord = [] 
    x2_coord = []
    coord_label = [] # For labeling points

    colors =['blue', 'green', 'red', 'orange']
    label_cnt = 0

    for word_class in class_to_words:
        for word in class_to_words[word_class]:

            word_idx = TERM_INDEX[word]

            word_x1, word_x2 = tsne_embeddings[word_idx,:]
            
            coord_label.append(word)
            x1_coord.append(word_x1)
            x2_coord.append(word_x2)
            class_id.append(label_cnt)

        label_cnt += 1

    fig, ax = plt.subplots(figsize=(10,10)) 
    
    for i, label in enumerate(coord_label):
        ax.annotate(label, (x1_coord[i],x2_coord[i]))

    plt.scatter(x1_coord, x2_coord, c=class_id, 
            cmap=matplotlib.colors.ListedColormap(colors))

    plt.savefig('tsne_embeds.pdf', dpi=150)
...
def BuildInferenceNetwork(x, l2_reg_val, is_training):
    ...
    with tf.variable_scope('', reuse=True):
        EMBEDDING_VAR = tf.get_variable('fully_connected/weights') # ** TASK 4: Move and set appropriately.
...
def main(argv):
    ...
    def step(lr=0.01, batch_size=100):
        ...
        for si in range(0, x_train.shape[0], batch_size):
            ...
            slice_x = SparseDropout(slice_x, keep_prob=0.3) # Modified
    ...
    for j in range(600): step(lr/2)
    for j in range(600): step(lr/4)
